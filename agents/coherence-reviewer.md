...
1. `mcp__plugin_gadriel_gadriel__findings_for_path` — cheapest; reads `.security/findings.json` for already-attached coherence findings.
2. `mcp__plugin_gadriel_gadriel__validate_buffer` — fast in-memory recheck when the user is editing a Pydantic model, a Zod schema, an OpenAPI spec, or a `response_format` block.
3. `mcp__plugin_gadriel_gadriel__validate_file` — full re-scan when the graph layer needs fresh reachability (does this schema actually flow into the consumer claimed by the rule?).
4. `mcp__plugin_gadriel_gadriel__fix_finding` — proposes a remediation. For Coherence, fixes often add `response_format`, tighten a schema, or insert a strict validator at the call site.
