...
1. `mcp__plugin_gadriel_gadriel__findings_for_path` — cheapest; reads `.security/findings.json` for findings already attached to the file.
2. `mcp__plugin_gadriel_gadriel__validate_buffer` — for fast in-memory re-validation when the user is editing an agent-config file (e.g. `agents.yaml`, `Crew()` definition, MCP server manifest).
3. `mcp__plugin_gadriel_gadriel__validate_file` — full re-scan when graph reachability matters (e.g. does this LLM call actually flow into a `shell.exec` sink?).
4. `mcp__plugin_gadriel_gadriel__fix_finding` — to propose a remediation. For Safety findings the proposed fix often wires a blocking input prompt or tightens a `temperature`/`max_tokens` setting; review carefully before presenting.
