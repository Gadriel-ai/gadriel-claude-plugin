...
1. `mcp__plugin_gadriel_gadriel__findings_for_path` — cheapest; reads `.security/findings.json` for attached teamwork findings.
2. `mcp__plugin_gadriel_gadriel__validate_buffer` — fast in-memory recheck when the user edits an MCP server manifest, a CrewAI `Crew()` block, a LangGraph definition, or a message-bus subscription handler.
3. `mcp__plugin_gadriel_gadriel__validate_file` — full re-scan when graph reachability matters (does the static call graph actually contain a cycle?).
4. `mcp__plugin_gadriel_gadriel__fix_finding` — proposes a remediation. For Teamwork, fixes often add a timeout, declare an ack schema, or break a wait cycle by inverting one side's ordering.
